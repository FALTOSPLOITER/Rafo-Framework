import requests
import sys
from threading import Thread, Lock
from queue import Queue
from colorama import Fore, Back, Style

try:
    from tqdm import tqdm
    TQDM_AVAILABLE = True
except ImportError:
    TQDM_AVAILABLE = False


def dirbust(host, wordlist, threads=8):

    q = Queue()
    list_lock = Lock()
    discovered_directories = []

    def scan_directories():
        while True:
            try:
                directory = q.get()
                url = f"http://{host}/{directory}"
                response = requests.get(url, timeout=5)
            except requests.ConnectionError:
                pass
            except KeyboardInterrupt:
                sys.exit('^C')
            except Exception as e:
                print(f'[{Fore.RED}!{Style.RESET_ALL}] Error: {Fore.RED}{e}{Style.RESET_ALL}')
            else:
                if response.status_code != 404:
                    print(f'[{Fore.GREEN}+{Style.RESET_ALL}] Discovered directory: {Fore.GREEN}{url}{Style.RESET_ALL}')
                    with list_lock:
                        discovered_directories.append(url)
            finally:
                q.task_done()

    print(f'[{Fore.YELLOW}?{Style.RESET_ALL}] Performing directory busting on {Fore.YELLOW}{host}{Style.RESET_ALL}...\nPress CTRL-C to cancel.')

    try:
        directories = open(wordlist).read().splitlines()

        if TQDM_AVAILABLE:
            for directory in tqdm(directories, desc='Queuing directories', unit='entry'):
                q.put(directory)
        else:
            print(f'This might take a while. Looking for directories in {Fore.YELLOW}{host}{Style.RESET_ALL}...')
            for directory in directories:
                q.put(directory)

        for _ in range(threads):
            worker = Thread(target=scan_directories)
            worker.daemon = True
            worker.start()

        q.join()

        if len(discovered_directories) > 0:
            if len(discovered_directories) == 1:
                print(f'\nScan completed. {Fore.GREEN}{len(discovered_directories)}{Style.RESET_ALL} directory was discovered.\n')
            else:
                print(f'\nScan completed. {Fore.GREEN}{len(discovered_directories)}{Style.RESET_ALL} directories were discovered.\n')
        else:
            print(f'Scan completed. No directories were discovered.\n')

    except KeyboardInterrupt:
        sys.exit('^C\n')
    except Exception as e:
        print(f'[{Fore.RED}!{Style.RESET_ALL}] Error: {Fore.RED}{e}{Style.RESET_ALL}')
